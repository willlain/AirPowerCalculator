...
function displayResultOption() {
    var html = "";
    var label_list = ["有効無効（＋撃墜率） 第一航空隊 1波 "," 2波","第二航空隊 1波 "," 2波","第三航空隊 1波 "," 2波"]
    var text_list = ["無効","20%","50%"];
    for (var i=0; i<6; i++) {
...
function displayList(category) {
    ...
    if (category == "ship") {
        var tab = tab_type_ship;
    } else {
        var tab = tab_type_equipment;
    }
    ...
    html_content += "<button type='button' class='btn btn-primary btn-sm' id='btn-select-" + category + "' onclick=\"selectItem(\'" + category + "\')\" disabled>OK</button>"
    html_content += "<button type='button' class='btn btn-primary btn-sm' onclick=\"$(\'#dialog-select-" + category + "\').dialog(\'close\')\">キャンセル</button>"
    html_content += "</div>"
    html_content += "</div>"
    ...
    for (var i=0; i<tab.length; i++) {
        if (!Array.isArray(tab[i])) {
            if (category == "ship") {
                var tab_type = "tab-" + category + "-" + i;
                var list_id = "list-" + category + "-" + tab[i];
                var type_name = data_ship_type[tab[i]].name;
                var id_list = data_ship_type[tab[i]].id;
                var data_id = data_ship_id;
                switch (type_name) {
                    case "軽空母":
                    case "軽巡洋艦":
                    case "重巡洋艦":
            ...
            for (var j=0; j<id_list.length; j++) {
                var data = data_id[id_list[j]];
                var info_id = "info-" + category + "-" + tab[i] + "-" + id_list[j];
            ...
            if (category == "ship") {
                var type_name = data_ship_type[tab[i][0]].name;
                var data_id = data_ship_id;
                switch (type_name) {
                    case "駆逐艦":
                        type_name = type_name.substr(0,2);
                        ...
                        Object.keys(data_ship_model).forEach(function(key){
                            tab_id.push("tab-" + category + "-" + i + "-" + tab[i][0]);
                            list_id.push("list-" + category + "-" + tab[i][0] + "-" + key);
                    ...
                    case "巡洋戦艦":
                        ...
                        for (var j=0; j<tab[i].length; j++) {
                            tab_id.push("tab-" + category + "-" + i + "-" + tab[i][0]);
                            list_id.push("list-" + category + "-" + tab[i][j]);
                ...
                var type_name = data_equipment_type_ship[tab[i][0]].name;
                var data_id = data_equipment_id_ship;
                switch (type_name) {
                    case "小口径主砲":
                        type_name = "主砲";
                        break;
                ...
                var id_list = [];
                for (var j=0; j<tab[i].length; j++) {
                    tab_id.push("tab-" + category + "-" + i + "-" + tab[i][j]);
                    list_id.push("list-" + category + "-" + tab[i][j]);
            ...
            for (var j=0; j<list_id.length; j++) {
                ...
                html_content += "<div class='list-group'>";   // 一覧リスト
                for (var k=0; k<id_list[j].length; k++) {
                    var data = data_id[id_list[j][k]];
                    var info_id = "info-" + category + "-" + tab[i][j] + "-" + id_list[j][k]
                    var item_id = list_id[j] + "-" + id_list[j][k]
